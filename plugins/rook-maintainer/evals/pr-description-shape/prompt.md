...
The maintainer's request that produced this change: "When the operator
crash-loops, `kubectl logs -f` dies at every restart and I lose exactly
the lines I need. I want a follow mode that survives restarts — add a
`rook logs` subcommand that reads the operator log."
