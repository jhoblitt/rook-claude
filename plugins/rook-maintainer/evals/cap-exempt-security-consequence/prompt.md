...
design candidates is settled, and the candidate whose cost is a
security consequence sorts LAST — every other design candidate
outranks it. Do not re-rank.
...
+	// ServiceUserSecretRef names that secret in any namespace; supersedes ServiceUserSecretName when set.
+	// +optional
+	ServiceUserSecretRef *v1.SecretReference `json:"serviceUserSecretRef,omitempty"`
...
@@ -204,7 +204,10 @@ func (c *clusterConfig) keystoneServiceUser(ks *cephv1.KeystoneSpec) (string, string, error) {
 	ns, name := c.store.Namespace, ks.ServiceUserSecretName
+	if ks.ServiceUserSecretRef != nil {
+		ns, name = ks.ServiceUserSecretRef.Namespace, ks.ServiceUserSecretRef.Name
+	}
 	s, err := c.context.Clientset.CoreV1().Secrets(ns).Get(ctx, name, metav1.GetOptions{})
 	if err != nil {
