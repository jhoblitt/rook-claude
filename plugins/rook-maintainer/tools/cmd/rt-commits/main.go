...
//
// --repo runs the git log itself over that checkout, read-only. --log consumes
// a dump of exactly that command instead, captured elsewhere:
//
//	git -c core.quotePath=false log --no-merges -M \
//	    --format=commit%x09%H%x09%aN%x09%aE%x09%aI --name-status > commits.log
//
...
func run() int {
	...
	fs.Usage = func() {
		fmt.Fprint(os.Stderr, "usage: rt-commits (--repo PATH | --log FILE) [--months N] [--now RFC3339] "+
			"[--out FILE | --json]\n")
		fs.PrintDefaults()
		fmt.Fprintf(os.Stderr, "\n--log consumes the output of:\n  %s\n", rtcommits.GitLogCommand())
	}
	...
	if *repo != "" {
		source = rtcommits.Source{Mode: "repo", Path: *repo}
		commits, source.Head, err = rtcommits.Log(context.Background(), *repo)
	} else {
