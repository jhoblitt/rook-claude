# Backport labels and mergify backport PRs

Which changes earn a `backport-release-X.Y` label is `references/backporting.md`;
this file is what follows once a change is eligible — which branches, the
label and who may add or remove it, and fixing the PR mergify opens.
...
This governs breadth, not whether: the confirmation gate in `backporting.md`'s table still
...
Blessing lifts `backporting.md`'s confirmation gate only as far as the
